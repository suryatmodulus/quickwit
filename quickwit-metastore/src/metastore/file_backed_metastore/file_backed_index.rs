...
impl FileBackedIndex {
    ...
    /// Returns whether at least one mutation occurred.
    ...
    fn delete_split(&mut self, split_id: &str) -> DeleteSplitOutcome {
        match self.splits.get(split_id).map(|split| split.split_state) {
            // Only `MarkedForDeletion` and `Staged` splits can be deleted
            Some(SplitState::MarkedForDeletion | SplitState::Staged) => {
