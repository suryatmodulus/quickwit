...
pub mod test_suite {
    ...
    pub async fn test_metastore_reset_checkpoint<MetastoreToTest: Metastore + DefaultForTest>() {
        ...
        let source_ids: Vec<String> = (0..3).map(|i| format!("{index_id}--source-{i}")).collect();
        let split_ids: Vec<String> = (0..3).map(|i| format!("{index_id}--split-{i}")).collect();
        ...
        assert!(!index_metadata
            .checkpoint
            .source_checkpoint(&source_ids[2])
            .unwrap()
            .is_empty());

        metastore.reset_index_checkpoint(&index_id).await.unwrap();
        assert!(metastore
            .index_metadata(&index_id)
            .await
            .unwrap()
